...
async function handleLogin() {
  ...
  try {
    ...
    // сохраняем всё, что может понадобиться другим компонентам
    localStorage.setItem('token', data.token || '')
    localStorage.setItem('auth_type', data.type || '')   // <-- добавили
    localStorage.setItem('user_type', data.type || '')   // <-- на всякий случай вторым ключом
    ...
    // уведомим родителя (если нужно без перезагрузки)
    emit('logged-in', { type: data.type, ministry: data.ministry || null })
    emit('close')

    // перезагрузим страницу, чтобы панель перечитала localStorage
    location.reload()
  } catch (e) {
...
.reg-tab[disabled] { pointer-events: none; opacity: 0.45; }
.modal-overlay { position: fixed; inset: 0; background: rgba(51,61,81,0.20); z-index: 9999;
...
.close-btn { position: absolute; top: 12px; right: 13px; font-size: 22px;
...
.modal-success {
  color: #36c900; font-size: 16px; text-align: center; margin: 26px 0 12px 0;
